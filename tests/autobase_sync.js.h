...
const topic='example'\
const timeout=200\
async function createPeer(){const sdk=await SDK.create({storage:false,autoJoin:true})\
goodbye(_=>sdk.close())\
return await createSwarm(sdk,topic)}\
test.onFinish(_=>process.exit(0))\
test('syncing events',async t=>{const peer1=await createPeer()\
const peer2=await createPeer()\
const eventPeer1={id:0,kind:1,pubkey:'peer1',content:'hello'}\
const eventPeer2={id:1,kind:1,pubkey:'peer2',content:'world'}\
